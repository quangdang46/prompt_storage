#!/usr/bin/env bash
# pst installer — production-grade curl-pipe installer
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/quangdang46/prompt_storage/main/install.sh | bash
...
Options:
  ...
  --system          Install to /usr/local/bin (needs sudo for pipe use)
  --easy-mode       Auto-add DEST to PATH in shell rc files
  --verify          Run '$BINARY_NAME --version' after install
  --from-source     Build from source with cargo instead of downloading
  --quiet, -q       Suppress informational output
  --uninstall       Remove installed binary and PATH entries
  -h, --help        Show this help
EOF
    exit 0
}

# === Args (support --flag value and --flag=value) ===
while [ $# -gt 0 ]; do
    case "$1" in
        --dest)        DEST="$2";      shift 2;;
        --dest=*)      DEST="${1#*=}"; shift;;
...
# === Platform detection → asset suffix mapping ===
detect_platform() {
    local os arch
    case "$(uname -s)" in
        Linux*)              os="linux";;
...
install_binary_atomic() {
    ...
    install -m 0755 "$src" "$tmp" && mv -f "$tmp" "$dest" || { rm -f "$tmp"; die "Failed to install binary"; }
}
...
build_from_source() {
    ...
    (cd "$TMP/src" && CARGO_TARGET_DIR="$TMP/target" cargo build --release -p pst)
...
main() {
    ...
    if [ "$FROM_SOURCE" -eq 0 ]; then
        ...
        # Release assets are named pst-<suffix> (no version prefix), matching release.yml
        ...
        if download_file "$url" "$TMP/$archive"; then
            ...
            [ -n "$bin" ] || bin=$(find "$TMP" -name "${BINARY_NAME}.exe" -type f 2>/dev/null | head -1)
    ...
    if [ "$VERIFY" -eq 1 ]; then
        "$DEST/$BINARY_NAME" --version || die "Installed binary failed to run"
    fi
    ...
    echo "    ${BINARY_NAME} new my-prompt --from -"
    echo "    ${BINARY_NAME} my-prompt"
}
